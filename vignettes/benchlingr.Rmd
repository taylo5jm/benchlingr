...
[Red box indicates the schema ID in the GUI. The column names of the input data frame must match the field names in the "Warehouse name" column.](vignettes/img/assay_result_schema_id_gui.jpg)
...
Now, you will need to assemble a data frame that can be uploaded to the Benchling system. The column names of your data frame must match the column names of the table in the data warehouse. In the example, we are working with, the column names are `plate`, `analytes`, and `file`. 
...
create_assay_results(conn=conn, client=client, df=df,
                     ...
                     fk_type="id", tenant="hemoshear-dev.benchling.com",
                     api_key=Sys.getenv("BENCHLING_DEV_API_KEY"))
```
