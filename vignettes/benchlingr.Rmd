...
It is important to keep the `schema` field in tables retrieved from the data warehouse, as many functions in `benchlingr` need it to work properly. 
...
*Show screenshot of table in Benchling*

The schema name for this table is `general_assay_file_submission`, so we can query the table to get the identifiers and file names for the blobs to download. 

```{r, eval=FALSE}
DBI::dbGetQuery(conn, "SELECT * FROM general_assay_file_submission")
```

`download_blobs` is a low-level wrapper for the Python SDK that downloads a set of blobs from Benchling as specified by their ID and file name. 
...
The `download_blobs_in_warehouse_table` function will download all files attached to a particular set of columns in a warehouse table. 

```{r, eval=FALSE}
download_blobs_in_warehouse_table(conn, df, column='excel', outdir="data")
```
