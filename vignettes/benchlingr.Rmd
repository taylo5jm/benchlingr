...
```{r, eval=TRUE}
DBI::dbGetQuery(conn, "SELECT * FROM plate") %>%
  tail()
```
...
![Custom "Analyte" entity](img/analyte-screenshot.png)
...
```{r, eval=TRUE}
DBI::dbGetQuery(conn, "SELECT * FROM analyte") %>%
  head()
```
...
```{r, eval=TRUE}
DBI::dbGetQuery(conn, "SELECT * FROM analyte$raw") %>%
  head()
```
...
Custom entities can have fields that are other entity types. For example, we defined a custom entity schema for cell cultures, which are connected to a "study" entity, which contains the metadata pertaining to an experiment, including a human-readable identifier, who performed the experiment, the objectives of the experiment, etc. 

![Custom "cell culture" entity schema](img/cell_culture_screenshot.png)
...
```
The code above did not work, because the warehouse table for "study" is not actually called `study`. This might occur when a custom schema is defined and later renamed, since the warehouse table names do not automatically get renamed (for good reason). It may also occur if the name of the field in one entity is not the same as the actual entity name. In this case, the field in the "cell culture" table is called "study", but the corresponding entity schema is actually called "study code". 

![study code entity](img/study_code_screenshot.png)



At this point, you might be wondering how one can find the warehouse table name for a particular entity, if the entity name can be out of sync with the warehouse table name. One way to find the warehouse table name of a custom entity is to simply look in the Benchling GUI itself. 




The identifier for any custom entity will start with the `bfi_`. To find more information about a custom entity 


```{r, eval=TRUE}
DBI::dbGetQuery(conn, "SELECT * FROM study_code WHERE id = 'bfi_ZAZ84eD3'")
```




At HemoShear Therapeutics, we started using the developer platform long before we implemented a process for notebook entry approvals, so it was necessary to use the "raw" tables in some cases. Even now, some notebook entries are never approved, because they function as data-dumping sites for staff members that operate a specific instrument. For example, our LC-MS team generates data for several scientific programs, so it made more sense for us to set them up with their own notebook entries for submitting data, instead of having them upload data to the notebook entry for each experiment. 


After verifying the database connection is valid, one can retrieve data using the `DBI::dbGetQuery` function.

```{r, eval=TRUE}
DBI::dbGetQuery(conn, "SELECT * FROM plate") %>% head
```
...
```{r}
DBI::dbGetQuery(conn, "SELECT * FROM study_plan_entry") %>%
  head()
```
...
```{r}
entry <- client$entries$get_entry_by_id("etr_FbZuzgv6")
```
...
## Reading unstructured tables in notebook entries. 
