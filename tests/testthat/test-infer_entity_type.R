...
test_that("Verify that infer_entity_type.R stops when given an invalid input for 
          entity_id", {
  expect_error(infer_entity_type(entity_id=character(0), tenant=tenant), 
               paste0("'entity_id' input is invalid. Must be a character ",
                      "vector with a length greater than 0."))
  expect_error(infer_entity_type(entity_id=NULL, tenant=tenant), 
               paste0("'entity_id' input is invalid. Must be a character ",
                      "vector with a length greater than 0."))
  expect_error(infer_entity_type(entity_id=NA, tenant=tenant), 
               paste0("'entity_id' input is invalid. Must be a character ",
                      "vector with a length greater than 0."))
