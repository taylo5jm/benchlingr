...
test_that("Verify that infer_entity_type.R provides the correct response when entity_id is missing or invalid", {
  ...
  expect_error(infer_entity_type(NA), "'entity_id' input is invalid.")
  expect_error(infer_entity_type(list(124,"seq_Cuf0bmCm", "bfi_Q13AlXkf")), "'entity_id' input contains an invalid identifier.")
  expect_error(infer_entity_type(list("seq_Cuf0bmCm", NA, "bfi_Q13AlXkf")), "'entity_id' input is invalid.")
})

test_that("Verify that infer_entity_type.R works in creating a proper character vector", {
