...
#' 
#' .submit_queries.R takes a named character vector with one or more elements where the names are the names 
#' of entity identifiers and the elements are each identifier's respective Single-Get API endpoint URL and uses 
#' them to call that identifier's Single-Get API endpoint in Benchling and extract the response contents.
#' @param api_queries A named character vector where the names are the names of entity identifiers and the 
#' elements are the Single-Get API endpoint URLs for each identifier.
#' @param benchling_api_key Benchling API key. 
#' @return A named list where the names are the names of entity identifiers and each element is a list that contains
