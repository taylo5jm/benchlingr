...
#' @keywords internal
.get_entry_table_by_name <- function(entry, table_name, table_indices, return_table_name) {
    ...
    days <- purrr::map_int(table_name_lookup, ~ names(.) %>%
                     as.integer)
    # Call the get_entry_table function over the positions/days we have.
    res <- purrr::map2(
      days, positions, ~ get_entry_table(
...
#' @keywords internal
.get_entry_table_by_location <- function(entry, day, table_position, table_indices,
                                          return_table_name) {
    ...
    res <- purrr::map(
      table_position, ~ get_entry_table(
...
#' tables <- get_entry_tables(entry, table_name='Experimental Conditions')
#' entry <- client$entries$get_entry_by_id("etr_aTa6eDWA")
#' tables <- get_entry_tables(entry, table_name=c("TableA", "TableB"))
#' }
#' @export
